...
# Check the two input files exist and are non-empty
if [ -s $TEXT_FILE ]; then
    ...
    if [ -s $FOMA ]; then
        ...
        total=$(grep "[a-zA-Z]" $TOKENIZED_TEXT_FILE | wc -l)
        ...
        echo "\nERROR: Could not access the specified FST, $FOMA"
    fi
else
    echo "\nERROR: Could not read from the text file input, $TEXT_FILE"
fi